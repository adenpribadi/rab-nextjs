    ...
    <div style={{ padding: '0', maxWidth: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1rem', minHeight: '100%' }}>
      ...
      <div className="mobile-header-wrap" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-secondary)', padding: '0.75rem 1.5rem', borderBottom: '1px solid var(--border-color)', borderRadius: '8px 8px 0 0' }}>
        <h1 style={{ fontSize: '1.2rem', margin: 0, fontWeight: 600, color: 'var(--text-primary)' }}>
          Master Materials & Wages (AHSP)
        </h1>
        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          Total: {materials.length} records
        </div>
      </div>
      ...
      <div className="responsive-grid" style={{ flex: 1, alignItems: 'flex-start' }}>
        ...
        <div className="glass-card" style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', flex: 1 }}>
                ...
                {materials.length === 0 ? (
                  ...
                  materials.map((item, index) => (
                    <tr key={item.id} style={{ borderBottom: '1px solid var(--border-color)', background: index % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                      ...
                      <td style={{ padding: '0.5rem 1rem', fontWeight: 500 }}>{item.name}</td>
                      ...
                      <td style={{ padding: '0.5rem 1rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                          <Link href={`/materials?editItem=${item.id}`} style={{ color: 'var(--text-muted)', textDecoration: 'none' }} title="Edit">
          ...
          {/* Add Form Area at the bottom */}
          <div style={{ padding: '1rem', borderTop: '1px solid var(--border-color)', background: 'var(--bg-tertiary)' }}>
            <h3 style={{ fontSize: '0.9rem', fontWeight: 600, margin: '0 0 0.75rem 0', color: 'var(--text-muted)' }}>Add New Material / Wage</h3>
            <form action={addMaterial} className="responsive-inline-form" style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
              <div style={{ flex: '2' }}>
                <input type="text" name="name" className="input-field" required placeholder="Name (e.g. Semen Portland, Tukang Batu)" style={{ padding: '0.4rem 0.5rem', fontSize: '0.8rem', height: '32px' }} />
              </div>
              <div style={{ flex: '1.5' }}>
                <select name="categoryId" className="input-field" style={{ padding: '0.4rem 0.5rem', fontSize: '0.8rem', height: '32px' }}>
                  <option value="">-- Optional Category --</option>
                  {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div style={{ width: '80px' }}>
                <input type="text" name="unit" className="input-field" required placeholder="Unit" style={{ padding: '0.4rem 0.5rem', fontSize: '0.8rem', height: '32px' }} />
              </div>
              <div style={{ flex: '1' }}>
                <input type="number" name="unitPrice" className="input-field" required placeholder="Unit Price (Rp)" style={{ padding: '0.4rem 0.5rem', fontSize: '0.8rem', height: '32px' }} />
              </div>
              <button type="submit" className="btn-primary" style={{ padding: '0 1.5rem', fontSize: '0.8rem', height: '32px', whiteSpace: 'nowrap' }}>
                + Add
              </button>
            </form>
          </div>
